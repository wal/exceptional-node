...
var Exceptional = {
  ...
  API_HOST: "api.getexceptional.com",
  API_PORT: 80,
  ...
  send_error: function(doc) {
    gzip(doc, 1, function(err, data) {
      ...
      var client  = HTTP.createClient(Exceptional.API_PORT, Exceptional.API_HOST);

      var headers = {
        'Host' : Exceptional.API_HOST,
      ...
      request.on('response', function (response) {
        if (response.statusCode === 200) {
          console.log("error successfully sent to exceptional");
        } else {
